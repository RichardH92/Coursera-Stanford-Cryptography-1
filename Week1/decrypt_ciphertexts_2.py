...
def find_spaces_in_ciphertext(ct_index):
	for i in range(0, min_length):
		if char_is_a_space(ct_index, i):
			print "(" + ct_index + ", " + i + ")"
			encryption_map[ct_index][i] = ' '

def char_is_a_space(ct_index, char_index):
	test_char = CIPHERTEXTS[ct_index][char_index]

	for i in range(0, len(CIPHERTEXTS)):

		if i != ct_index:
			xor_chr = (ord(test_char) ^ ord(CIPHERTEXTS[i][char_index]))
			xor_chr_space = (xor_chr ^ ord(' '))

			print (xor_chr << 4)
			print (xor_chr_space << 4)

			if (xor_chr << 4) == (xor_chr_space << 4):
				print "TRUE"
...
xord_str = strxor(CIPHERTEXTS[0], CIPHERTEXTS[1])
